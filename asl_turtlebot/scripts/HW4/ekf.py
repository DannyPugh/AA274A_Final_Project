    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        for i in range(z_raw.shape[1]):
            ...
            for j in range(hs.shape[1]):
                ...
                v_ij = np.array([angle_diff(z[0], h[0]), z[1] - h[1]])

                S_ij = H @ self.Sigma @ H.T + Q_raw[i]

                d_ij = v_ij @ np.linalg.inv(S_ij) @ v_ij

                if d_ij < d_min and d_ij < self.g ** 2:
                    d_min = d_ij
                    v_min = v_ij
                    H_min = H
                    Q_min = Q_raw[i]
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        g3, Gx3, Gu3 = tb.compute_dynamics(self.x[:3], u, dt)
        g[:3] = g3
        Gx[:3, :3] = Gx3
        Gu[:3, :] = Gu3
        ########## Code ends here ##########
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx3 = tb.transform_line_to_scanner_frame(np.array([alpha, r]),
                                                         self.x[:3],
            ...
            x, y, theta = self.x[:3]
            x_base, y_base, theta_base = self.tf_base_to_camera
            ...
            if j >= 2:
                ...
                Hx[1, idx_j] = -x_cam * (-np.sin(-alpha))*(-1) - (y_cam) * np.cos(-alpha)
            ########## Code ends here ##########
