...
class ParticleFilter(object):
    ...
    def resample(self, xs, ws):
        ...
        us = ws.sum() * (np.linspace(0, 1, self.M, endpoint=False) + r)
        idxs = np.searchsorted(np.cumsum(ws), us)
        self.xs = xs[idxs]
        self.ws = ws[idxs]
...
class MonteCarloLocalization(ParticleFilter):
    ...
    def transition_model(self, us, dt):
        ...
        gate = np.abs(om) > EPSILON_OMEGA

        g_f = dt * np.column_stack([V * cos, V * sin, np.zeros(self.M)])
        g_t = np.column_stack([ V/om * (sin_n - sin), -V/om * (cos_n - cos), om * dt])
        g = self.xs + np.where(gate[:, np.newaxis], g_t, g_f)
        ########## Code ends here ##########
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        ws = scipy.stats.multivariate_normal.pdf(list(vs), mean=None, cov=Q)
        ########## Code ends here ##########
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        hs = self.compute_predicted_measurements() # [M x 2 x J]
        M = self.M
        I = z_raw.shape[1]
        J = self.map_lines.shape[1]

        vs = np.zeros((M, I, 2))
        # v = h [M x 2 x J] - z [2 x I]
        for m in range(M):
            for i in range(I):
                z = z_raw[:, i]

                v_i = np.array([angle_diff(z[0], hs[m,0,:]), z[1] - hs[m,1,:]])
                d_i = np.einsum('ij,ij->j', v_i, np.linalg.solve(Q_raw[i], v_i))
                v_min = v_i[:,np.argmin(d_i)].flatten()
    ...
    def compute_predicted_measurements(self):
        ...
        # TODO(avati): vectorize this
        for m in range(M):
            x = self.xs[m]
            for j in range(J):
                line = self.map_lines[:, j]
                h, _ = tb.transform_line_to_scanner_frame(line, x, tf_base_to_camera)
