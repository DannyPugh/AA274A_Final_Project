...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1):
        ...
        if not self.is_free(self.x_init):
            for N in range(100):
                N_res = N * self.resolution
                for i in range(-N,N):
                    if self.is_free(self.snap_to_grid((self.x_init[0] + i * self.resolution, self.x_init[1] - N_res))):
                        self.x_init = self.snap_to_grid((self.x_init[0] + i * self.resolution, self.x_init[1] - N_res))
                        break
                    if self.is_free(self.snap_to_grid((self.x_init[0] + i * self.resolution, self.x_init[1] + N_res))):
                        self.x_init = self.snap_to_grid((self.x_init[0] + i * self.resolution, self.x_init[1] + N_res))
                        break
                    if self.is_free(self.snap_to_grid((self.x_init[0] - N_res, self.x_init[1] + i * self.resolution))):
                        self.x_init = self.snap_to_grid((self.x_init[0] - N_res, self.x_init[1] + i * self.resolution))
                        break
                    if self.is_free(self.snap_to_grid((self.x_init[0] + N_res, self.x_init[1] + i * self.resolution))):
                        self.x_init = self.snap_to_grid((self.x_init[0] + N_res, self.x_init[1] + i * self.resolution))
                        break
        ...
        if not self.is_free(self.x_goal):
            for N in range(10):
                N_res = N * self.resolution
                for i in range(-N,N):
                    if self.is_free(self.snap_to_grid((self.x_goal[0] + i * self.resolution, self.x_goal[1] - N_res))):
                        self.x_goal = self.snap_to_grid((self.x_goal[0] + i * self.resolution, self.x_goal[1] - N_res))
                        break
                    if self.is_free(self.snap_to_grid((self.x_goal[0] + i * self.resolution, self.x_goal[1] + N_res))):
                        self.x_goal = self.snap_to_grid((self.x_goal[0] + i * self.resolution, self.x_goal[1] + N_res))
                        break
                    if self.is_free(self.snap_to_grid((self.x_goal[0] - N_res, self.x_goal[1] + i * self.resolution))):
                        self.x_goal = self.snap_to_grid((self.x_goal[0] - N_res, self.x_goal[1] + i * self.resolution))
                        break
                    if self.is_free(self.snap_to_grid((self.x_goal[0] + N_res, self.x_goal[1] + i * self.resolution))):
                        self.x_goal = self.snap_to_grid((self.x_goal[0] + N_res, self.x_goal[1] + i * self.resolution))
                        break
    ...
    def is_free(self, x):
        ...
        """
        ########## Code starts here ##########
        for l, h, v in zip(self.statespace_lo, self.statespace_hi, x):
            if v < l or v > h:
                return False
    ...
    def get_neighbors(self, x):
        ...
        ########## Code starts here ##########
        for i in [-1, 0, 1]:
            #if x[0] + self.resolution * i < self.statespace_lo[0]:
            #    continue
            #if x[0] + self.resolution * i > self.statespace_hi[0]:
            #    continue
            for j in [-1, 0, 1]:
                #if x[1] + self.resolution * i < self.statespace_lo[1]:
                #    continue
                #if x[1] + self.resolution * i > self.statespace_hi[1]:
                #    continue

                if i == 0 and j == 0:
                    continue

                y = self.snap_to_grid((x[0] + i * self.resolution,
        ...
        while len(self.open_set) > 0:
            ...
            for x_neigh in self.get_neighbors(x_curr):
                if x_neigh in self.closed_set:
                    continue
                tent_CtA = self.cost_to_arrive[x_curr] + self.distance(x_neigh, x_curr)
                if x_neigh not in self.open_set:
                    self.open_set.add(x_neigh)
                elif self.cost_to_arrive[x_neigh] < tent_CtA:
                    continue
                self.came_from[x_neigh] = x_curr
                self.cost_to_arrive[x_neigh] = tent_CtA
                self.est_cost_through[x_neigh] = tent_CtA + self.distance(x_neigh, self.x_goal)
    ...
    def is_free(self, x):
        ...
        for obs in self.obstacles:
            ...
            for dim in range(len(x)):
                #CORNER CLIPPING FIX: add a buffer ie +ROBOT_WIDTH to the edge of the obs to prevent robot from driving too close
